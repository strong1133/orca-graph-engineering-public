...
`GraphNode`는 routing 값을 부분적으로 override할 수 있습니다. `effectiveRouting`은 각 키를 독립적으로 해석하므로, 예를 들어 graph의 project/model을 상속하면서 node에서 session만 바꿀 수 있습니다. Task 단건 실행의 `per_project` 모드는 이 routing DTO를 각 target locator마다 독립적으로 가지며, 모든 route를 먼저 live-attest한 뒤에만 세션을 생성합니다.
