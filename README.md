...
Orca 안에서 실행 그래프를 설계하고 Task·Todo를 관리하며 각 Task를 Orca 프로젝트, 기존 세션, 모델로 라우팅하는 오픈소스 플러그인입니다. 외부 서비스 없이 로컬 JSON만으로 완전히 동작하고, 선택적으로 공개 Data Source contract나 임의 JSON을 연결할 수 있습니다.
...
- Task/Todo 목록의 Draft·Meta·Domain·Milestone 통합 검색, 범위·상태 필터, Domain/Milestone/상태/우선순위 그룹화와 정렬
...
- Task/Todo 검색은 제목·ID·태그·사람 Draft·Meta Draft·Domain·Milestone을 함께 찾습니다. 목록은 Domain, Milestone, 상태, 우선순위별로 그룹화할 수 있고 Domain/Milestone을 정확히 필터링할 수 있습니다.
...
- hard delete 대신 Domain/Milestone/Task 보관과 Todo 취소를 사용합니다. 활성 하위 Milestone·Task·Todo가 남은 Domain이나 Milestone은 보관할 수 없습니다.
...
먼저 `대상 갱신` 후 `실행 계획`으로 Task와 하위 그래프의 실제 project/session/model/reasoning 및 run 계보를 확인하십시오. dry-run과 실제 `실행`은 모두 root의 선택된 condition branch, AND/OR join과 graph-call routing 결합을 따라 전체 execution plan을 먼저 계산하고, 선택된 모든 descendant Task의 pure route를 같은 resolver로 검사합니다. graph-call 순환·누락·보관·root hop limit, session/project/worktree, model allow-list/agent family/reasoning capability를 어떤 run record나 Orca call보다 먼저 검사하므로 invalid dry-run도 run history를 남기지 않습니다. dry-run이 생략하는 것은 `worktree ps`, `terminal list`, `terminal wait` 같은 live attestation과 terminal mutation뿐이며 성공한 pure plan은 Orca call 0회로 planned run을 기록합니다.

구조 오류, 선택하지 않았거나 존재하지 않는 조건 분기, 위험 권한 조합, loop guard 누락, idempotency·승인 gate 누락, 민감/제한 데이터의 network 정책 위반, token budget 초과도 같은 경계에서 차단됩니다. panel의 실행 modal도 idempotency와 sensitive-network 위반을 bridge와 같은 error로 차단합니다. 비가역 노드는 선택된 모든 non-loop 실행 경로를 지배하는 approved human gate가 있어야 하며, unrelated/downstream/pending gate는 승인 경계로 인정되지 않습니다. 로컬 브리지가 아직 지원하지 않는 loop 재진입도 live-run 전에 fail-closed로 차단하며 dry-run 계획과 정적 검사는 계속 제공합니다.
