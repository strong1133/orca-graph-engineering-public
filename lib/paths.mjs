...
/*
 * 패널 스냅샷이 실려 나가는 파일. 기본값은 이 패키지가 Orca에 넘기는 패널이다.
 *
 * 격리된 런타임으로 CLI를 돌릴 때(테스트가 그렇다) 이 값을 함께 옮기지 않으면,
 * 남의 런타임 데이터가 설치된 플러그인의 패널을 덮어써 사용자가 만든 Graph·Task가
 * 패널에서 사라진 것처럼 보인다.
 */
export const panelPath = process.env.ORCA_GRAPH_PANEL_PATH
  ? path.resolve(process.env.ORCA_GRAPH_PANEL_PATH)
